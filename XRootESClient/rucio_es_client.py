	...
	def aggregate_success(self, day):
		...
		while True:
			...
			if input["operation"] == "STORE":
				...
				xfer_matrix[idx1][idx2][0] += size/1000000
		...
		for i in range(len(keys)):
			for j in range(len(keys)):
				...
				new_entry = {
					...
					"source" : keys[i],
					"destination" : keys[j],
					"file_size" : xfer_matrix[i][j][0]*1000000,
			...
			for j in range(len(keys)):
				for k in [0, 1]:
					...
					new_entry = {
						...
						"destination" : keys[j],
						# "reason" : "tx_error",
						"count" : xfer_matrix[k][i][j]
						}
					if k == 0:
						new_entry["reason"] = "tx_error"
					elif k == 1:
						new_entry["reason"] = "rx_error"
					yield new_entry

		# for i in range(len(keys)): # added
		# 	for j in range(len(keys)): # added
		# 		if xfer_matrix[1][i][j] == 0:
		# 			print('GOT TO INDEX 1')
		# 			continue
		# 		new_entry = {
		# 			"name" : f"{keys[i]}_to_{keys[j]}",
		# 			"source" : keys[i],
